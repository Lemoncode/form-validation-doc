...
In this structure it can define all the validations associated to a given form (field and record validations).

This allow us to define all validation associated to a given form in a declarative way.

Properties:

- **field**: optional entry, here it can define all field validations associated to a given form
  (see [FieldValidationSchema](/api#fieldvalidationschema) for more information.)

- **record**: optional entry, here it can define all record validations associated to a given form
...
In this structure it can define all field validations associated to a given form. For each field, it has to add a validator list.
...
- **customArgs**: optional, here it can pass any additional configuration, needed to setup the validator (e.g. in a max-length validation the length value, or in a regEx validator the regEx pattern).
- **message**: optional, in this parameter it can override the default message(s) that the validator generates.

#### Examples:

You could see these examples in [fonk examples](https://github.com/Lemoncode/fonk/tree/master/examples/docs/api).

**Schema Field validations**

Let's say it has a login form, and the following record:
...
It wants to validate that both user and password are required field, a validation
...
In this structure it can define all record validations associated to a given form. For each record, it has to add a validator list.
...
Whenever we validate all form record entries ([ValidateRecord](/api#validaterecord)),
...
Whenever it validates a form with [ValidateForm](/api#validateform) (trigger
...
Allows us to fire all the validations associated to given field, this method is async and it returns a promise with a
...
- Starts iterating over all the associated fields that have validations defined in the ValidationSchema:
  - The validation engine will start sequentially firing the list of validations associated to that field in them form valition schema.
  - It will stop iterating through all validations associated to the given field:
