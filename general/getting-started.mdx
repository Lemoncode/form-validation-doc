...
We are gong to start by adding some lines of code to define a login validation record:
...
Let's get started with Fonk: is time to create the validation schema:
...
Since the record fields user is empty, we will get the following output from the _console.log(validationResult)_:
...
already informaed and _passowrd_ field is empty):
