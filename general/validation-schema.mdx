...
What happens if we need to add addional context information to the validator function, for instance passing a custom error message ? The **ValidationSchema** supports two signatures whenever we add a validations:

- The short definition: the one we have been using, is just adding the validator function we wan to use.
...
[Check live demo](https://codesandbox.io/s/github/lemoncode/fonk/tree/master/examples/vanilla/js/nested-field)
...
Let's create a simple login form, and as validations the user name must be required, and a valid github account (we will check against github api).
...
- [Plain Vanilla JS and TS](https://codesandbox.io/s/github/lemoncode/fonk/tree/master/examples/vanilla).
- [React + React Final Form JS and TS](https://codesandbox.io/s/github/lemoncode/fonk/tree/master/examples/react-final-form).
