...
> What do we mean by synchronous? That is any validation that executes some code and returns a straightforward result,
...
- You can override the validation message just for a given ValidationSchema (for instance if you use
  the _pattern_ (regex) validator you may want for an specific field in a validation schema to return
...
We could just go the easy way and change the validation message to:
...
_Wow ! That was awesome ! but how can I customize this message if I need internazionalization?_ You can
...
> This validator is already available as a third party plugin, you can find it in the following [link](TODO).
...
We got a form that displays the total of a given shopping order:
...
Once the user hits on submit order and before sending the request to the server we want to check the following rule:
...
We only want to trigger this once the user has finished picking up the product he wants to buy (we don't want to annoy him meanwhile he is in the
...
> If you need to implement internazionalization or customize messages you can follow the same approach
...
Now that you have learn how to build synchronous custom validators is time to learn how to implement [asynchronous custom valators](/validators/custom-validators-async).
