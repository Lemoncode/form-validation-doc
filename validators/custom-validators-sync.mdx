...
_IBAN country code prefix must be {{customArgs.countryCode}}_
...
- let defaultMessage = 'IBAN does not belong to Spain';
+ let defaultMessage = 'IBAN does not belong to {{customArgs.countryCode}}';
 export const setErrorMessage = message => (defaultMessage = message);
...
override the message and just use the same interpolation sequence _{{customArgs.CountryCode}}_ where applies.
...
Now that you have learn how to build syncrhonous custom validators is time to learn how to implement [asynchronous custom validators](TODO).
