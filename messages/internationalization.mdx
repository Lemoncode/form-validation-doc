...
As we learned in previous section, we can customize error message globally using the `setErrorMessage` method provided by validators. So, we only need to provide the translated message when language changes:
...
If we are using `React`, we can use `useTranslation` hook and acces to the `t` method:
...
## Local error message translation

This time, we will use `message` property available in `validationSchema`. Each time language is updated, we will provide the translated message to this property, that is, we need to refresh the form validation instance:
